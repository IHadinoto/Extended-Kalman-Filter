...
stdDevSpeed = 0.3 ;   % We simulate a lot of error!  (very difficult case). 
...
sdev_rangeMeasurement = 0.35 ;                  % std. of noise which pollutes in range measurements =. 0.15m
...
%sdev_angleMeasurement_assumed = 0.10;
...
% (i.e. no updates, just predictions ; so it will be as a simple dead-reckoning)

% at least 1 landmark would allow a better localization than a simple
...
for i=1:Li     % loop
    ...
    [nDetectedLandmarks,MasuredRanges,MasuredAngles,IDs]=GetObservationMeasurements();
    ...
    if nDetectedLandmarks>0     % any laser data and detected landmarks?
        ...
        for u=1:nDetectedLandmarks
            ...
            eDA = atan2(eDY,eDX)-Xe(3)+pi/2;
    
        
            % here is it. "H". I reuse some previous calculations.
            H = [-eDX/eDD -eDY/eDD 0;eDY/eDD^2 -eDX/eDD^2 -1] ;   % Jacobian of h(X); size 2x3
            ...
            z  = [MasuredRanges(u)-ExpectedRange;wrapToPi(MasuredAngles(u)-ExpectedAngle)] ;      

            % ------ covariance of the noise/uncetainty in the measurements
            R = [sdev_rangeMeasurement^2 0;0 (sdev_angleMeasurement)^2];
...
function [speed,GyroZ] = SimuControl(~,t)
...
function [nDetectedLandmarks,MasuredRanges,MasuredAngles,IDs]=GetObservationMeasurements(~)
    ...
    % simulated platform.
        ...
        if (nDetectedLandmarks<1)       % no detected landmarks...
            MasuredRanges=[]; MasuredAngles=[]; IDs=[];    return ; 
        end
        ...
        MasuredRanges = RealRanges+noiseInRange;
        % so MasuredRanges are the measurements polluted with
        % noise. I get the "perfect measurements" from the simulated
        % platform.
        noiseInAngle= ContextSimulation.sdev_angleMeasurement*randn(size(RealAngles));
        MasuredAngles = RealAngles+noiseInAngle;
