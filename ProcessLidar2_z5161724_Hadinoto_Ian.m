function [xcoor,ycoor,xseg,yseg,xrob,yrob,xgf,ygf,xgfseg,ygfseg,linx,liny,linx2,liny2,linx3,liny3,linx4,liny4,linx5,liny5]=ProcessLidar2_z5161724_Hadinoto_Ian(r,I,X,pt,theta,Mapx,Mapy);
    ...
    % Global Coordinates
    for i=1:length(I)
        Matrix = Rthetainv*[xcoor(i);ycoor(i)+0.46]+[X(1);X(2)];
    ...
    for i=1:length(segment)
        Matrix = Rthetainv*[xseg(i);yseg(i)+0.46]+[X(1);X(2)];
    ...
    % Robot angle shift
    for i=1:length(robx)
        Matrix = Rthetainv*[robx(i);roby(i)+0.46]+[X(1);X(2)];
