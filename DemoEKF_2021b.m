% DemoEKF.m - version 2021.T1
% MRTN4010 - T1.2021
% Example using Extended Kalman Filter (EKF) for the localization problem, using LIDAR range observations.
% Read and analyze this progam; it may be used for solving part of Project 3 (although that one is based on real data).
...
% Even frequently using "just" one range measurement (related to just one landmark) the estimator is
...
% (dt=30ms), however we do not assume we get measurements (for the observations) at that sample
% rate (that situation would be great!, but it is not always possible). We assume observations have a slower sample rate. This
% is to show that even for a poor system (observing at low rate, and just few landmarks) the estimation process is still good and feasible. 
...
% Note4:  IMPORTANT
% a) You may need to modify the EKF update stage in order to deal with the
% cases:   Bearing Only and Range+Bearing (gives more info than just range measurements).
...
% and gyroZ). Follow the approach presented in the lecture notes.
...
%    b)  Wz (yaw/heading angular rate), measured through  IMU's gyro Z.


% by J. Guivant for MRTN4010's students, Session T1.2021



function DemoEKF_2021b()


% .....................................................
% Here I define the magnitude of the "noises", which I assume are present, 
% polluting mesurements, models, etc.  
% (we are simulating those noises, so we can try whatever case we want.) 

% Standard deviation of the error in the angular rate sensor  (i.e. std of noise in gyro's measurements). 
stdDevGyro = 2*pi/180 ;        %"actual"
stdDevGyro_assumed =stdDevGyro ; %presumed/assumed one.

% 1 degrees/second , standard deviation of the gyros' noise
...
stdDevSpeed = 0.10 ;   % We simulate a lot of error!  (very difficult case). 
stdDevSpeed_assumed = 0.15 ;


% Speed-meter's error = 0.10m/sec; actually, a low quality speed sensor! (it
% may be the case, on certain terrains in which platform's traction is not good.)

% ... errors in the range measurements (15cm, standard dev.)
sdev_rangeMeasurement = 0.15 ;                  % std. of noise which pollutes in range measurements =. 0.15m
sdev_rangeMeasurement_assumed =0.20;     % std (which we presume/assume) of noise polluting range measurements, 

% This is more than the error you would have with our laser scanner. Am I right? (the answer may be useful for Project 3)


SmallBiasGyro = 0.05*pi/180 ;   % we can even simulate the effect of a minor bias in the gyroscope readings.
% just 0.05degrees/sec
% it will kill the dead reckoning.
% .....................................................

% some parameters, for the simulation context.
Dt=0.05 ;                       % "sample time", 50ms
Li = 6000 ;                     % "experiment" duration, in iterations 
DtObservations=0.100 ;          % LiDAR;s "sample time" (for observations), ~5Hz, approximately
...
n_usedLanmarks = 1;    %it must be : 0 < n_usedLanmarks <5, but you can modify the function "CreateSomeMap()" to extend it.
% just to create some landmarks
% if you specify "n_usedLanmarks = 0", you would not have observations
% (i.e. no updates, just predictions ; soit will be as a simple dead-reckoning)

% at least 1 landnark would allow a better localization that a simple
...
% period of time of Dt (30ms) we introduce uncertainty of 0.01m standard deviation on X, 
...
% the lecturer notes about how to improve our propossed Q matrix. 

% I assume Q is allways the same (but that may not be the case). 
Q = diag( [ (0.01)^2 ,(0.01)^2 , (1*pi/180)^2]) ;
% Q matrix. Represent the covariance of the uncertainty about the process model.
% this is a poor tunning!  --> you need to improve it.

%e.g. some improvement here. But you can refine it further.
Q(3,3) = (stdDevGyro_assumed*Dt)^2;
Q(1,1) = (stdDevSpeed_assumed*Dt)^2; Q(2,2) =Q(1,1) ;
...
Noisy_GyroZ=Noisy_GyroZ+SmallBiasGyro;
    
    % run our "open-loop" estimation (just dead-reckoning), using noisy inputs.
    ...
    % this J is explanined in "[AAS]_L05_Localization Problem Using
    % EKF[2021].pdf"
    
    % then I calculate the new coaeriance matrix, due to the prediction, P(K+1|K) = J*P(K|K)*J'+Q ;
    P = J*P*J'+Q ;
   ...
   % The predition step, for this iteration, is done.
    ...
    if nDetectedLandmarks>0,     % any laser data and detected landmarks?
        ...
        for u=1:nDetectedLandmarks,
            ...
            R = sdev_rangeMeasurement_assumed^2;
...
function [nDetectedLandmarks,MasuredRanges,IDs]=GetObservationMeasurements(map)
 ...
 % Read the simulatated system's state.   THis is the real value of the
 % stare. In real applications, this is never known (that is why we try to
 % estimate it !)
...
if (map.nLandmarks>0),   
    plot(map.landmarks(:,1),map.landmarks(:,2),'+r','markersize',15,'linewidth',3) ;
...
figure(3) ; clf ; 
Xe=Xe_History;
subplot(311) ; plot(Xreal_History(1,:)-Xe(1,:)) ;ylabel('x-xe (m)') ;grid on;
title('Performance EKF (should be good)') ;
subplot(312) ; plot(Xreal_History(2,:)-Xe(2,:)) ;ylabel('y-ye (m)') ;grid on;
subplot(313) ; plot(180/pi*(Xreal_History(3,:)-Xe(3,:))) ;ylabel('heading error (deg)') ;grid on;xlabel('time (k)');
...
% In the same way we record the expected values and the "real values", you
% may record the marginal variances (diagonal elements of the P matrix), to
% verify that the discrepancy between estimated and "real" (ground truth)
% are consistent with the statistics reported by the estimator.

% this is what I do in the animated simulation, in class, at the end of
% that simulation.
    ...
    function [Xe,P]=DoUpdate2(P,Xe,H,R,z)
            % Some intermediate steps for the EKF (as presented in the lecture notes)
            HP=H*P;
            S = R + HP*H' ;
            iS=inv(S);                 
            ...
            Xe = Xe+K*z ;       % update the  expected value
            P = P-K*HP ;       % update the Covariance % i.e. "P = P-P*H'*iS*H*P"  )
...
%  Jose Guivant -  AAS -Session 1.2021
